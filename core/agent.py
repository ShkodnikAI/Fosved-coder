...
SYSTEM_PROMPT_TEMPLATE = """Ты MindCoder — AI-ассистент для разработки проекта.
...
SYSTEM_PROMPT_INJECTION_TEMPLATE = """Ты MindCoder — AI-ассистент для разработки проекта.
...
async def execute_tool(name: str, arguments: dict, project_path: str | None, websocket) -> str:
    ...
    try:
        if name == "read_file":
            ...
            try:
                asyncio.create_task(save_tool_usage(None, "", "", "read_file", json.dumps({"path": path}), "done", duration_ms=int((time.time()-_tool_start)*1000), result_length=len(content)))
            except Exception as _telemetry_err:
                logger.log("telemetry_save_failed", level="warning", source="agent",
                           error=str(_telemetry_err)[:200])
            return content
            ...
            try:
                asyncio.create_task(save_tool_usage(None, "", "", "write_file", json.dumps({"path": path}), "done", duration_ms=int((time.time()-_tool_start)*1000), result_length=len(content)))
            except Exception as _telemetry_err:
                logger.log("telemetry_save_failed", level="warning", source="agent",
                           error=str(_telemetry_err)[:200])
            ...
            try:
                asyncio.create_task(save_tool_usage(None, "", "", "list_files", json.dumps({"path": rel_path}), "done", duration_ms=int((time.time()-_tool_start)*1000)))
            except Exception as _telemetry_err:
                logger.log("telemetry_save_failed", level="warning", source="agent",
                           error=str(_telemetry_err)[:200])
            return result
            ...
            try:
                asyncio.create_task(save_tool_usage(None, "", "", "search_files", json.dumps({"pattern": pattern}), "done", duration_ms=int((time.time()-_tool_start)*1000)))
            except Exception as _telemetry_err:
                logger.log("telemetry_save_failed", level="warning", source="agent",
                           error=str(_telemetry_err)[:200])
            return result
            ...
            try:
                asyncio.create_task(save_tool_usage(None, "", "", "execute_command", json.dumps({"command": command[:200]}), "done", duration_ms=int((time.time()-_tool_start)*1000)))
            except Exception as _telemetry_err:
                logger.log("telemetry_save_failed", level="warning", source="agent",
                           error=str(_telemetry_err)[:200])
            ...
            try:
                asyncio.create_task(save_tool_usage(None, "", "", "git_commit_push", json.dumps({"message": message}), "done", duration_ms=int((time.time()-_tool_start)*1000)))
            except Exception as _telemetry_err:
                logger.log("telemetry_save_failed", level="warning", source="agent",
                           error=str(_telemetry_err)[:200])
            # Stage all
            ...
            try:
                asyncio.create_task(save_tool_usage(None, "", "", "git_clone", json.dumps({"repo_url": repo_url}), "done", duration_ms=int((time.time()-_tool_start)*1000)))
            except Exception as _telemetry_err:
                logger.log("telemetry_save_failed", level="warning", source="agent",
                           error=str(_telemetry_err)[:200])
            project_token = await get_project_token_by_path(project_path)
        ...
        try:
            asyncio.create_task(save_tool_usage(None, "", "", name, "", "error", duration_ms=int((time.time()-_tool_start)*1000)))
        except Exception as _telemetry_err:
            logger.log("telemetry_save_failed", level="warning", source="agent",
                       error=str(_telemetry_err)[:200])
...
async def stream_llm_response(prompt: str, history: list, websocket,
    ...
    for iteration in range(max_tool_iterations):
        ...
        try:
            ...
            try:
                asyncio.create_task(save_model_usage(None, "", model, "", "", total_tokens=tokens, duration_ms=int(duration), success=True))
            except Exception as _telemetry_err:
                logger.log("telemetry_save_failed", level="warning", source="agent",
                           error=str(_telemetry_err)[:200])
            ...
            try:
                asyncio.create_task(save_model_usage(None, "", model, "", "", duration_ms=int(duration), success=False))
            except Exception as _telemetry_err:
                logger.log("telemetry_save_failed", level="warning", source="agent",
                           error=str(_telemetry_err)[:200])
    ...
    try:
        asyncio.create_task(save_model_usage(None, "", model, "", "", total_tokens=len(full_response)//4, duration_ms=int((time.time()-start_time)*1000), tool_calls_count=max_tool_iterations, success=True))
    except Exception as _telemetry_err:
        logger.log("telemetry_save_failed", level="warning", source="agent",
                   error=str(_telemetry_err)[:200])
...
HUB_SYSTEM_PROMPT = """Ты MindCoder — AI-ассистент для подготовки проектов.
...
- **САМОСТОЯТЕЛЬНО подключать нужные скиллы** в зависимости от типа задачи

## АВТОМАТИЧЕСКОЕ ПОДКЛЮЧЕНИЕ СКИЛЛОВ

Ты ОБЯЗАН анализировать задачу пользователя и ПОДКЛЮЧАТЬ нужные скиллы.
Используй формат: <skill name="skill_id">краткое описание зачем нужен</skill>

Можешь подключить НЕСКОЛЬКО скиллов в одном ответе, если задача комплексная.
Подключай скиллы ДО начала работы, как только понял суть задачи.

### Доступные скиллы:

**AI и мультимедиа:**
- llm — Чат и генерация текста через языковые модели (LLM)
- vlm — Анализ изображений и визуального контента (Vision Language Model)
- asr — Распознавание речи, speech-to-text
- tts — Синтез речи, text-to-speech
- image-gen — Генерация изображений по текстовому описанию (логотипы, мокапы, иллюстрации)
- video-gen — Генерация и обработка видео

**Документы:**
- pdf — Создание, редактирование и анализ PDF документов
- docx — Создание и редактирование Word документов
- xlsx — Создание и анализ таблиц Excel
- ppt — Создание презентаций PowerPoint

**Веб и исследования:**
- web-search — Поиск информации в интернете
- web-reader — Извлечение контента с веб-страниц
- browser — Автоматизация браузера (headless)
- aminer-academic-search — Поиск научных статей и публикаций (AMiner)

**Разработка:**
- coding-agent — AI-агент для написания кода
- charts — Графики, диаграммы, ER-схемы, визуализации данных
- finance — Финансовые данные и аналитика рынка
- skill-creator — Создание новых навыков

**UI/UX дизайн:**
- form-design — Формы: валидация, мульти-шаг, accessibility
- error-handling-ux — Профилактика и обработка ошибок в UI
- loading-states — Skeleton screens, optimistic UI, индикаторы прогресса
- state-machine — Конечные автоматы для сложных UI-состояний
- micro-interaction-spec — Микроанимации, триггеры, фидбэк
- animation-principles — Принципы UI-анимации: easing, duration, timing
- responsive-design — Адаптивные layout и breakpoint стратегии
- navigation-patterns — Типы навигации и выбор паттерна
- onboarding-design — First-run опыт, онбординг, time-to-value
- feedback-patterns — Системный фидбэк для действий пользователя
- accessibility-audit — WCAG аудит и инклюзивный дизайн
- dark-mode-design — Стратегия тёмной темы и контраст
- search-ux — Поиск: автокомплит, zero-results, фильтры
- gesture-patterns — Жесты и touch-взаимодействие
- data-visualization-design — Принципы визуализации данных

### Примеры когда подключать:
- "Сделай отчёт" → pdf, docx, xlsx
- "Найди информацию" → web-search, web-reader
- "Создай дашборд с графиками" → charts, data-visualization-design
- "Разработай лендинг" → responsive-design, navigation-patterns, animation-principles
- "Сделай презентацию" → ppt
- "Проанализируй конкурентов" → web-search, finance
- "Спроектируй мобильное приложение" → gesture-patterns, onboarding-design, form-design
...
- ПОДКЛЮЧАЙ СКИЛЛЫ ПРОАКТИВНО — не жди пока пользователь попросит
"""
...
QUESTIONNAIRE_SYSTEM_PROMPT = """Ты MindCoder — аналитик требований проекта.
