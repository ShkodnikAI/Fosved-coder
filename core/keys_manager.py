...
class KeysManager:
    ...
    Статусы провайдера:
      - valid        — ключ работает, проверен (зелёный)
      - rate_limited — временная проблема: лимит, таймаут, 404, connection error.
                       Имеет status_ts; если age > RATE_LIMITED_TTL → background_revalidation_loop
                       пробует ещё раз. Используется как валидный для попыток.
      - invalid      — ключ явно невалиден (401/400). Скрывается из UI и не используется.
                       Имеет status_ts; если age > INVALID_TTL → редкая повторная проверка.

    Probed-кэш (self._probed_model_ids / _failed_probe_ids) — источник правды на стороне клиента;
    сервер их использует как подсказку.
    """
